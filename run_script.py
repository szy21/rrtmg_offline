...
def main():
    ...
    for tau0 in tau0s:
        for lon in lons[0:1]:
            ...
            f_rrtm = glob.glob('/export/data1/zhaoyi/GCMForcedLES/'+tau0+'/250m/Output.'+casename+'.4x/stats/RRTM_control*')
            if not f:
                print casename+': does not exist!'
            elif not f_rrtm:
            #if True:
                run_str = 'python main.py input/'+casename+'.in > log/'+casename+'.log'
                print run_str
